...
def get_irreps(mol,C):
    from pyscf import symm
    if(mol.symmetry==False): irr = ['E']*mol.nao_nr()
    else:
      irrep_dict = {}
      for x,y in zip(mol.irrep_name,mol.irrep_id):
          irrep_dict[y] = x
      irr = [irrep_dict[x] for x in symm.label_orb_symm(mol,mol.irrep_id,mol.symm_orb,C)]
    return irr
...
def compute_integrals(atom,
    ...
    try:
        ...
        mol = gto.Mole(atom=atom, unit=unit, basis=basis,
                       max_memory=max_memory, verbose=4, output=output)
        mol.symmetry = symgroup
        mol.charge = charge
        mol.spin = spin
        mol.build() #parse_arg=False)
        q_mol = _calculate_integrals(mol, hf_method, conv_tol, max_cycle, init_guess, outfile)
